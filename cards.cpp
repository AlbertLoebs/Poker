...
int main() {
    ...
    std::vector<std::string> yourHand;
    std::vector<std::string> otherHand;
    std::vector<std::string> communityHand;

    // Deal hands to players
    dealCards(deck, yourHand, otherHand);

    // Deal community cards
    dealCommunityCards(deck, communityHand);

    std::cout << "\nDetermining the winner...\n" << std::endl;

    // Determine the winner
    determineWinner(yourHand, otherHand, communityHand);
